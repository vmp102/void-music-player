...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "q", "ctrl+c":
			saveConfig(m)
			return m, tea.Quit
		case "j": if m.cursor < len(m.folders)-1 { m.cursor++ }
		case "k": if m.cursor > 0 { m.cursor-- }
		case "n":
			if len(m.displayQueue) > 0 {
		...
		case "s":
			...
			if len(m.masterQueue) > 0 {
				...
				for i, p := range m.displayQueue {
					if p == m.currentPath { m.queueIdx = i; break }
				}
			}
		case "enter":
		...
		case "p":
			if ctrl != nil {
				speaker.Lock(); ctrl.Paused = !ctrl.Paused
				m.playing = !ctrl.Paused; speaker.Unlock()
			}
		}
	}
	return m, nil
